...
function functionTemperrature(tab2 , nombre){
  let tab = [];
  let valeurInferieure = [];
  let i = 0 ;
  let currentElement ;

  let valreturn = document.createElement("p")

  while (i < tab.length) {
    currentElement = tab2.length-1
    if (currentElement < nombre) {
      push.valeurInferieure(tab2[0]);
      valreturn.textContent = valeurInferieure
  ...
  console.log(functionTemperrature(tab2 , 12))
